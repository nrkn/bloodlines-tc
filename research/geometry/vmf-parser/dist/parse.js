...
function parse(text, options) {
    ...
    options = {
        types: (typeof options === "boolean") ? options // backward compatibility with the old boolean param
    ...
    var getNextLine = function () {
        if (sublines && sublines.length) {
            ...
            if (!odd)
                _subline = _subline.trim(); // we need to trim the line if outside of quoted value
            return _subline;
        }
        var _line = lines[++i];
    ...
    while ((line = getNextLine()) !== false) {
        ...
        if (line[0] == "}") {
            if (Array.isArray(stack[stack.length - 2]))
                stack.pop(); // if the element above is an array, we need to pop twice
            stack.pop();
            continue;
        }
        ...
        while (true) {
            ...
            if (val === undefined) {
                ...
                if (options.conditionals !== undefined && Array.isArray(options.conditionals) && m[9]) {
                    ...
                    if (!ok) {
                        ...
                        if (!line || line[0] == '/')
                            break; // break if there is nothing else (of interest) left in this line
                        continue;
                    }
                }
                if (options.types) {
            ...
            if (expect_bracket)
                break; // there was just key, no value, the next line should contain bracket (to go one level deeper)
            line = line.replace(m[0], "").trim();
            if (!line || line[0] == '/')
                break; // break if there is nothing else (of interest) left in this line
            line = line.replace(/^\s*\[\!?\$[A-Z0-9]+(?:(?:[\|]{2}|[\&]{2})\!?\$[A-Z0-9]+)*\]/, "").trim(); // ignore conditionals
            if (!line || line[0] == '/')
                break; // again; if there's nothing left after skipping the conditional
        }
    }
    if (stack.length != 1)
...
function stringify(obj, options) {
    ...
    options = {
        pretty: (typeof options === "boolean") ? options // backward compatibility with the old boolean param
