...
const start = async () => {
  ...
  const parseSide = (t: string) => {
    ...
    const B = b.split(' ').map(s => Number(s)) as Tuple3
    const C = c.split(' ').map(s => Number(s)) as Tuple3
  ...
  // hack - while we're here let's dump the flats - that's the most interesting
  // for now
  ...
  const rectsIntersect = (a: Rect, b: Rect) => {
    ...
    if (b.x >= a.x && b.x <= right && b.y >= a.y && b.y <= bottom) return true

    return false
  }


  const rectToFlatMap = new Map<string, FlatData[]>()
  ...
  const rects = Array.from(rectToFlatMap.keys()).map(
    s => JSON.parse(s) as Rect
  )
  ...
  for (const a of rects) {
    for (const b of rects) {
      ...
      if (rectsIntersect(a, b)) intersects++
    }
  ...
  for (const key of rectToFlatMap.keys()) {
    ...
    for (const flat of flats) {
      flatsSvg += `<rect class="id_${flat.id}" data-material="${flat.material}" data-z="${flat.z}" x="${flat.rect.x}" y="${flat.rect.y}" width="${flat.rect.width}" height="${flat.rect.height}" fill="rgb(${flat.color.join()
  ...
  writeFileSync('./pawnshop-flats.svg', flatsSvg, 'utf8')

  writeFileSync('./pawnshop.html', html, 'utf8')
  writeFileSync('./pawnshop-flats.json', JSON.stringify(flats, null, 2), 'utf8')

  // wild - let's just make a 2d grid from the flats now

  const xKeys = new Set<number>()
  ...
  type GridRect = Rect & { i: number, j: number, flats: FlatData[] }
  ...
  for (let j = 0; j < yIndices.length - 1; j++) {
    ...
    for (let i = 0; i < xIndices.length - 1; i++) {
      ...
      const gr: Rect = { x, y, width, height }
  ...
  const svgHeader = (viewBox: string) =>
    `<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="${viewBox}">`

  const attrStr = (key: string, value: string) => `${key}="${value}"`

  const objToAttr = (obj: Record<string, string | number>) =>
    Object.entries(obj).map(([k, v]) => attrStr(k, String(v))).join(' ')
  ...
  const gridRectToSvg = (rect: GridRect, strat: 'max' | 'min' | 'avg') => {
    if (rect.flats.length === 0) return ''

    const { i, j, x, y, width, height } = rect

    const zIndices = rect.flats.map(f => f.z)

    if (strat === 'max' && zIndices.length > 2) levelOverLevel++

    const fminZ = Math.min(...zIndices)
    const fmaxZ = Math.max(...zIndices)

    if (fminZ > maxHeight || fmaxZ > maxHeight) return ''

    let z = minZ

    if (strat === 'max') z = fmaxZ
    if (strat === 'min') z = fminZ
    if (strat === 'avg') z = (fmaxZ + fminZ) / 2
  ...
  writeFileSync('./pawn-grid-max.svg', gridSvgMax, 'utf8')
  ...
  writeFileSync('./pawn-grid-min.svg', gridSvgMin, 'utf8')
  ...
  writeFileSync('./pawn-grid-avg.svg', gridSvgAvg, 'utf8')
  ...
  const getZ = (i: number, j: number) => {
    ...
    if (i >= xIndices.length) return
    if (j >= yIndices.length) return
  ...
  writeFileSync('./pawn-grid-points.svg', gridSvgPoints, 'utf8')
}
